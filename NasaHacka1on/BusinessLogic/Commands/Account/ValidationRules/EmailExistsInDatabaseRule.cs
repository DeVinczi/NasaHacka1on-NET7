...
internal class EmailExistsInDatabaseRule : IValidationRule<string>
{
    public const string EmailHasBeenAlreadyUsed = "email-has-been-already-used";

    private readonly CommunityCodeHubDataContext _dataContext;

    public EmailExistsInDatabaseRule(CommunityCodeHubDataContext dataContext)
    {
    ...
    public async Task<Gybs.IResult> ValidateAsync(string email)
    {
        ...
        if (exists)
        {
            return Result.Failure(ResultErrorKeys.InvalidParameter, EmailHasBeenAlreadyUsed);
        }
